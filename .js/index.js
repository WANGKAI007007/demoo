/* function getMid() {
	try {
	  return window.external.GetSpecialData(window.external.twGetSecurityID(window)).split(',')[3];
	} catch (e) {}
	try {
	  return window.wdextcmd.GetMid();
	} catch (e) {}
	try {
	  return window.external.GetMID(window.external.GetSID(window));
	} catch (e) {}
	return '';
};
const haoMonitor = window.haoMonitor; // haopv
const radarMonitor = window.radarMonitor; // 雷达
const { screen } = window;
const getBase = haoMonitor.data.getBase;

// 重写 getBase 为 haopv 增加一些全局字段
haoMonitor.data.getBase = function () {
  const base = getBase();
  const urlParams = new URLSearchParams(window.location.search.substring(1));
  return {
    ...base,
    qd: urlParams.get('src') || '_default',
    __hmid: getMid(),
    screen: `${screen.width}_${screen.height}`,
  };
};
...
const monitor = {
  getTrack() {
    haoMonitor.getTrack();
    radarMonitor.getTrack();
  },
  log() {
    haoMonitor.log.apply(haoMonitor, arguments);
    radarMonitor.log.apply(radarMonitor, arguments);
  },
}; */
function display() {
  var img = document.getElementById('scanInner');
...
function goApp() {
	//monitor.log({ cId: 'bk: phone' , c: '手机生成计算题'}, 'click')
	window.open('https://fanqie.com')
}
/**
 * @todo 1. 实现强制不借、进位
 * @todo 2. 实现生成带余数的除法
...
var app = new Vue({
  ...
  updated: function(){
    var iframe = parent.document.getElementById('mathpaperframe');
    if(iframe){
      var frameHeight = document.body.scrollHeight + this.frameBottom;
      frameHeight = frameHeight < 807 ? 807 : frameHeight;
